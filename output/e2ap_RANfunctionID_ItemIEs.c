...
{
   int stat = 0;
   RTXCTXPUSHTYPENAME (pctxt, "RANfunctionID_ItemIEs");
...

   {
      ...
      switch (pvalue->value.t) {
         case T_E2AP_PDU_Contents_RANfunctionID_ItemIEs_:
            RTXCTXPUSHELEMNAME (pctxt, "e2ap-E2nodeComponentConfigAddition-Item");
            stat = asn1PE_e2ap_RANfunctionID-Item (pctxt, &pvalue->value.u_RANfunctionID_ItemIEs_.value);
            if (stat != 0) return LOG_RTERR (pctxt, stat);
            RTXCTXPOPELEMNAME (pctxt);
            break;
