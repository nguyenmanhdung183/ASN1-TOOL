...
int asn1PE_e2ap_RANfunctionsID_List (OSCTXT* pctxt, e2ap_RANfunctionsID_List* pvalue)
{
   int stat = 0;
   OSRTDListNode* pnode;
   e2ap_RANfunctionID-ItemIEs* pdata;

   RTXCTXPUSHTYPENAME (pctxt, "RANfunctionsID_List");
   ...
   for (xx1 = 0; pnode != 0 && xx1 < pvalue->count; pnode = pnode->next, xx1++) {
      pdata = (e2ap_RANfunctionID-ItemIEs*) pnode->data;
      RTXCTXPUSHARRAYELEMNAME (pctxt, "SEQUENCE", xx1);

      stat = asn1PE_e2ap_RANfunctionID-ItemIEs (pctxt, pdata);
      if (stat != 0) return LOG_RTERR (pctxt, stat);
...
int asn1PD_e2ap_RANfunctionsID_List (OSCTXT* pctxt, e2ap_RANfunctionsID_List** ppvalue)
{
   ...
   for (xx1 = 0; xx1 < count; xx1++) {
      ...
      pdata = rtxMemAllocType (pctxt, e2ap_RANfunctionID-ItemIEs);
      if (!pdata) return LOG_RTERR (pctxt, RTERR_NOMEM);

      asn1Init_e2ap_RANfunctionID-ItemIEs (pdata);

      stat = asn1PD_e2ap_RANfunctionID-ItemIEs (pctxt, pdata);
      if (stat != 0) {
...
void asn1Free_e2ap_RANfunctionsID_List (OSCTXT* pctxt, e2ap_RANfunctionsID_List* pvalue)
{
   ...
   while (pnode) {
      e2ap_RANfunctionID-ItemIEs* pdata = (e2ap_RANfunctionID-ItemIEs*) pnode->data;
      if (pdata) {
         asn1Free_e2ap_RANfunctionID-ItemIEs (pctxt, pdata);
         rtxMemFreePtr (pctxt, pdata);
      }
      pnode = pnode->next;
   }
